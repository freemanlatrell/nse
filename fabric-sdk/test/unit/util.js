...
function getMember(username, password, client, t, peerId, mspid, orgName, caId) {
    ...
    return client.getUserContext(username, true)
            ...
            if (orgName) {
                cryptoSuite = client.newCryptoSuite({path: module.exports.storePathForOrg(orgName)});
            } else {
            ...
            var cop = new copService(caUrl, tlsOptions, caId, cryptoSuite);
...
function getAdmin(client, t, mspid, orgName, keystorePath, signcertsPath) {
    var keyPath = path.join(__dirname, '../', keystorePath);
    var keyPEM = Buffer.from(readAllFiles(keyPath)[0]).toString();
    var certPath = path.join(__dirname, '../', signcertsPath);
    var certPEM = readAllFiles(certPath)[0];

    if (orgName) {
        client.newCryptoSuite({path: module.exports.storePathForOrg(orgName)});
    }
...
module.exports.getSubmitter = function(client, test, peerOrgAdmin, peerId, orgName, caId) {
	if (arguments.length < 2) throw new Error('"client" and "test" are both required parameters');

	var peerAdmin, mspid, id, caName, enrollSecret, keystorePath, signcertsPath;
    ...
    for (let key in ORGS.peers) {
        if (ORGS.peers[key].name === peerId) {
            keystorePath = ORGS.peers[key].key_store;
            signcertsPath = ORGS.peers[key].signcerts;
        }
    }
    ...
    if (peerAdmin){
        ...
        return getAdmin(client, test, mspid, orgName, keystorePath, signcertsPath);
    } else {
        return getMember('admin', enrollSecret, client, test, id, mspid, orgName, caId);
    }
};
